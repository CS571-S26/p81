...
export default function ChapterDetailPage() {
  const { book_id, chapter_number } = useParams();
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/books/${book_id}/chapters/${chapter_number}`)
      .then(res => res.json())
      .then(data => {
        // console.log("book data: ", data.content);   // Use for debugging
        // console.log("analysis data: ", data.analysis); // Use for debugging
        setChapter(data.content);
        setAnalysis(data.analysis);
        setBook(data.meta_data);
      });
  }, [book_id, chapter_number]);

  // Testing for chapters list
  useEffect(() => {
    fetch(`${API_URL}/books/${book_id}`)
      .then(res => res.json())
      .then(c => {
        setChaptersLength(c.chapters.length)
      })
  }, [book_id]);
  ...
  if (!chapter || !analysis) return <p>Loading...</p>;

  const chapterSplit = chapter.text.split("\n");
  ...
  function renderChapterWithTooltip(text) {
  ...
  return (
    <>
      <Container className="my-4">
        ...
        <br/>
        <p>Chapter {chapter.chapter_number}: {chapter.title}</p>
        <br/>
        <Stack>
      ...
      <Container>
        {chapter.chapter_number < chaptersLength
          ? <Button onClick={() => navigate(`/books/${book_id}/chapters/${parseInt(chapter_number)+1}`)}>Next Chapter</Button>
          : <Button disabled={true}>End of Book</Button>}
      </Container>
      {hoveredWord && (
